...
class ReservationValidator implements ValidatorInterface
{
    public function validate(array $data): ValidationResult
    {
        ...
        // salle_id
        if (!isset($data['salle_id']) || !v::intType()->positive()->validate($data['salle_id'])) {
        ...
        // date_debut
        if (!isset($data['date_debut']) || !v::dateTime('Y-m-d H:i:s')->validate($data['date_debut'])) {
            $errors['date_debut'][] = 'La date de début est invalide.';
        }

        // date_fin
        if (!isset($data['date_fin']) || !v::dateTime('Y-m-d H:i:s')->validate($data['date_fin'])) {
